...
class CoreApiRestClient {
    constructor(sessionToken, apiKey, options) {
        ...
        this.requestTimeoutMs = Math.max(0, asFiniteNumber(options === null || options === void 0 ? void 0 : options.requestTimeoutMs, constants_1.DEFAULT_START_SESSION_REQUEST_TIMEOUT_MS));
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                switch (response.status) {
                    ...
                    default:
                        throw new ClientError_1.ClientError('Unknown error when starting session', ClientError_1.ErrorCode.CLIENT_ERROR_CODE_SERVER_ERROR, response.status, { cause: data.message });
                }
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    const clientError = new ClientError_1.ClientError('Failed to get session token', isAuthenticationError
                        ? ClientError_1.ErrorCode.CLIENT_ERROR_CODE_AUTHENTICATION_ERROR
                        : response.status >= 400 && response.status < 500
                            ? ClientError_1.ErrorCode.CLIENT_ERROR_CODE_VALIDATION_ERROR
                            : ClientError_1.ErrorCode.CLIENT_ERROR_CODE_SERVER_ERROR, response.status, { cause: responseMessage });
                ...
                if (typeof data.sessionToken !== 'string' || !data.sessionToken) {
                    throw new ClientError_1.ClientError('Failed to get session token', ClientError_1.ErrorCode.CLIENT_ERROR_CODE_SERVER_ERROR, 500, { cause: 'Response did not include a session token' });
                }
                ...
                throw new ClientError_1.ClientError('Failed to get session token', ClientError_1.ErrorCode.CLIENT_ERROR_CODE_SERVER_ERROR, 500, { cause: error instanceof Error ? error.message : String(error) });
            }
        });
    }
    getApiUrl() {
...
function resolveRetryOptions(options) {
    ...
    const maxAttempts = Math.max(1, Math.floor(asFiniteNumber(options === null || options === void 0 ? void 0 : options.maxAttempts, constants_1.DEFAULT_START_SESSION_MAX_ATTEMPTS)));
    const initialBackoffMs = Math.max(0, asFiniteNumber(options === null || options === void 0 ? void 0 : options.initialBackoffMs, constants_1.DEFAULT_START_SESSION_INITIAL_BACKOFF_MS));
    const maxBackoffMs = Math.max(initialBackoffMs, asFiniteNumber(options === null || options === void 0 ? void 0 : options.maxBackoffMs, constants_1.DEFAULT_START_SESSION_MAX_BACKOFF_MS));
