  ...
  solveHandlerRef.current = (step) => {
    ...
    try {
      ...
      convoCmdRef.current.sendUserMessage(
        '[ПЛАТФОРМА] Ребёнок ответил ПРАВИЛЬНО. ОБЯЗАТЕЛЬНО начни с одной короткой похвалы (разными словами, не «молодец» каждый раз), потом веди дальше сама — когда расскажешь связку, покажи следующее (show_trainer / next_slide). НЕ спрашивай «готов?» и не жди, пока ребёнок попросит, но и не части — спокойно и по-доброму.',
      )
    } catch {/* noop */}
  }
