...
- [x] **Phase 6.5: EU WS-прокси (Frankfurt) — РФ-юзеры без VPN** — Реализовано на **h2.nexus** (Hetzner забанил аккаунт, DigitalOcean отклонил карту, h2.nexus принимает СБП). Node WS-proxy + nginx + Let's Encrypt + HMAC handshake. Voice works in browser end-to-end (user-confirmed 2026-05-12 with VPN; without-VPN UAT pending user retest). Bonus: transcript chat UI added. См. `.planning/phases/06.5-hetzner-proxy/06.5-SUMMARY.md`.
- [x] **Phase 7: HTML-тренажёр — контракт data-атрибутов + event bus** — Ребёнок решает задания, бот видит ввод и подсвечивает элементы.
- [x] **Phase 8: Агент управляет доской и тренажёром (client tools + contextual updates)** — Nataly сама решает когда рисовать на доске («объясни в столбик» в разговоре → агент дёргает `draw_explanation` через 11labs Client Tool → наш фронт зовёт `/api/draw`). Симметрично — события тренажёра (`trainer:answer_submitted`, `idle_15s`) идут в агента через `sendContextualUpdate`, и она реагирует в живом разговоре. **REVISED** 2026-05-12: исходный план был "двухуровневая LLM с отдельным Pedagogical server-watcher" — упрощён до встроенного function-calling агента 11labs. Полную двухуровневую архитектуру оставляем как Phase 8.5 если client-tools-path окажется недостаточным. **DONE** 2026-05-13: 8 планов в 6 волн, агент в 11labs cloud прописан с 6 client tools (live PATCH verified). PED-02 visibility + consecutive-mistakes triggers wired. Полный E2E на vitest + Playwright. Surfaced 11labs API migration (workspace-tools API) — restore-script переписан под новый POST /tools + tool_ids контракт.
- [ ] **Phase 8.5: Учебный материал — контент тренажёра по программе 5 класса** — Заполнение `public/trainer-configs/` JSON-конфигами для всех 12 тем v1 (натуральные числа, дроби, проценты, площади, уравнения, среднее арифметическое...). Параллельный трек, не блокирует код. **NEW** 2026-05-12.
- [ ] **Phase 8.6: Focus-group fixes — учительница представляется + менее роботизированный голос + ровные подсветки** — После UAT 2026-05-14 с 2 пятиклассниками: (1) учительница теперь представляется по имени «Наташа» в first_message + системном промпте; (2) TTS settings подкручены (stability 0.30→0.20, добавлен style: 0.60) — менее монотонный голос; (3) [PENDING] корректные координаты для highlight_region в board scenes — кривые красные выделения на сложении в столбик. Layout-redesign + Lottie + resize-able panels — отдельная работа в Phase 11. **NEW** 2026-05-14.
- [x] **Phase 9: 2D Lottie аватар учителя** — На странице урока виден живой аватар, переключающий состояния по событиям голосового агента. (CSS-эмодзи v1 готов; полноценные Lottie с эмоциями — Phase 11.)
...
- [ ] **Phase 11: UI redesign + Lottie-аватарки + Stroke-drawing + SSML-синхронизация** — **EXPANDED** 2026-05-12. Перестройка layout (доска уменьшается, тренажёр расширяется, аватар получает свой выделенный slot); 5-6 настоящих Lottie-аватарок учителя, переключаются по `voice:state` + `avatar:emotion`; и финальная синхронизация голос+рука+текст (исходный scope фазы).
...
- [x] 08-05-PLAN.md — Wave 2 BoardPanel + LessonShell: board:draw_request/board:clear_request subscriptions + trainerConfig thread to VoicePanel
- [x] 08-06-PLAN.md — Wave 3 periodic checkpoint: lib/periodic-checkpoint + VoicePanel useEffect on conversation.status
- [x] 08-07-PLAN.md — Wave 4 TrainerPanel progress UI: current-task ring + "N из M" counter + checkmarks (D-02; HTM-01 extension)
- [x] 08-08-PLAN.md — Wave 5 PED-02 proactive triggers (visibility + consecutive mistakes) + E2E suite + 11labs cloud agent restored (workspace-tools API migration adapted in-flight)
