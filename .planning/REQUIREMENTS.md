...
### ACC-01 — Авторизация ребёнка через личную ссылку
- **Source**: derived from v1 scope lock (2026-05-09).
- **Description**: Ребёнок получает уникальную личную ссылку (магическая ссылка / token-based), переходит по ней и попадает в свой ЛК без публичной формы регистрации.
- **Acceptance criteria**:
  - Ссылка содержит токен, идентифицирующий конкретного ребёнка.
  - При первом переходе — токен валидируется, создаётся session.
  - Session сохраняется (cookie / localStorage) для последующих посещений с того же устройства.
  - Истёкший / отозванный токен → понятная страница «ссылка недействительна, обратитесь к репетитору».
  - Никаких email/password регистраций со стороны ребёнка — это инвариант (см. INV-zero-install).
...
- **Source**: REQ-zero-install-invariant (`.planning/intel/requirements.md`).
- **Description**: Ребёнок открыл сайт → разрешил микрофон → начал урок. Никаких VPN, расширений, плагинов, регистраций.
- **Acceptance criteria**:
  - Никаких клиентских VPN, расширений браузера, плагинов, дополнительных программ.
  - Никаких регистраций со стороны ребёнка (вход — только по личной ссылке, см. ACC-01).
  - Любое решение в любой фазе, требующее от ребёнка дополнительного действия (кроме «разрешить микрофон»), отвергается.
  - Применимо к: фазе ЛК, фазе lesson page, фазе deploy.
