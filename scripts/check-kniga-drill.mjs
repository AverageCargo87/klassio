...
// 🔴 ПОЗИЦИЯ ВЕРНОГО ОТВЕТА (второй разбор Владимира, 13.08)
//  «Правильный ответ стоит первым в 21 из 30 заданий (70 %)… ребёнок за вечер выучит
//  „жми первое“ и получит высокий балл.» Цифра сошлась до единицы, и держалась она
//  ровно потому, что проверки на неё не было: приёмка смотрела, ЕСТЬ ли верный вариант,
//  и не смотрела, ГДЕ он стоит. Требование записано в методичке 08.08 («позиция
//  правильного — случайная») — теперь оно проверяется, а не только заявлено.
const сВариантами = drill.pages.flatMap((p) => [...(p.tasks || []), ...(p.spare || [])])
  .filter((t) => t.options && t.kind !== 'match')
const поМестам = {}
for (const t of сВариантами) поМестам[t.ok] = (поМестам[t.ok] || 0) + 1
const самоеЧастое = Math.max(...Object.values(поМестам))
const доля = Math.round((100 * самоеЧастое) / сВариантами.length)
say(доля <= 50, 'верный ответ разложен по местам, а не сидит на одном: '
  + Object.keys(поМестам).sort().map((k) => (+k + 1) + '-я — ' + поМестам[k]).join(' · ')
  + ' (чаще всего ' + доля + '%, потолок 50%)')
const первым = Math.round((100 * (поМестам[0] || 0)) / сВариантами.length)
say(первым <= 45, 'стратегия «жми первое» не работает: первым верный стоит в ' + первым + '% заданий')
// и идентификаторы уникальны: по ним будет храниться, что ребёнок уже знает
const идентификаторы = drill.pages.flatMap((p) => [...(p.tasks || []), ...(p.spare || [])]).map((t) => t.id)
const дубли = [...new Set(идентификаторы.filter((x, i) => идентификаторы.indexOf(x) !== i))]
say(!дубли.length, 'у каждого задания свой идентификатор' + (дубли.length ? ' — ДУБЛИ: ' + дубли.join(', ') : ' (' + идентификаторы.length + ')'))
...
if (t1) {
  ...
  // 🔴 ПЕРЕВЁРНУТО 13.08 по второму разбору Владимира: «пункт становится зелёным после
  //  верного ответа БЕЗ подсказки». Проверка выше сама берёт подсказку (она смотрит, не
  //  выдаёт ли та ответ) — и ТРЕБОВАЛА, чтобы следующий ответ всё равно засчитали «с
  //  первой попытки». То есть закрепляла ровно ту завышенную цифру, которая уезжала в
  //  итоги и в письмо родителю. Теперь требуем обратного: подсказка стоит попытки.
  say(s1.спервой === okStats.спервой,
    'ответ ПОСЛЕ подсказки не идёт в «сам с первой попытки» (' + s1.спервой + ')')
  // ⚠️ Снимок okStats берётся УЖЕ ПОСЛЕ подсказки — счётчик к этому моменту вырос.
  say(s1.подсказок >= 1 && s1.подсказок === okStats.подсказок,
    'зато подсказка посчитана отдельно (' + s1.подсказок + ')')
  say(s1.знаю === okStats.знаю, 'и в «знаю» такой пункт не попал (' + s1.знаю + ')')
}
