...
// ⚠️ 13.08: считать «за четыре такта до конца» стало нельзя — в сценарии появились
// реплики самой Ани (пояснения новых слов), а карточки «запомни» рождаются только из
// тактов ЧТЕНИЯ. Отсчитываем от них, иначе на разборе пересказа не по чему судить.
const jumped = await page.evaluate(() => {
  const sc = window.__kniga.fullScript().filter((b) => b.page === 120)
  const чтение = sc.filter((b) => b.kind === 'read' || b.kind === 'spot')
  const цель = чтение[Math.max(0, чтение.length - 4)] || sc[Math.max(0, sc.length - 4)]
  return window.__kniga.goBeat(цель.n)
})
...
if (c0) {
  ...
  // 🔴 13.08: закон формы не изменился — фраза по-прежнему обязана быть учебниковой.
  //  Но по разбору Владимира («она говорит „она делилась на три части“ — кто делился,
  //  непонятно») в выжимке возвращают подлежащее вместо местоимения. Такая фраза не
  //  дословна с первых слов, а с четвёртого — дословна; это и проверяем.
  const свои = S ? S.пункты.filter((p) => !p.вучебнике) : [{}]
  say(свои.length === 0, 'КАЖДАЯ фраза самари — из учебника, своего не добавлено'
    + (свои.length ? ': «' + String(свои[0].фраза).slice(0, 60) + '»' : ''))
  const восст = S ? S.пункты.filter((p) => p.подлежащее) : []
  say(восст.length > 0, 'а где учебник начинал с местоимения — подлежащее возвращено ('
    + восст.length + ': ' + восст.map((p) => p.термин).join(' · ') + ')')
...
if (c1) {
  // 🔴 ПЕРЕВЁРНУТО 13.08 по разбору Владимира: «она воспроизводит, и здесь ещё параллельно
  //  подчёркивает и читает одно и то же — ты должен одновременно и налево и направо
  //  смотреть, это неправильно… справа должна идти суммаризация».
  //  Раньше проверка ТРЕБОВАЛА подсветку на странице во время чтения выжимки. Теперь
  //  требуем обратного: пока читают справа, слева пусто.
  const hl = await page.evaluate(() => document.querySelectorAll('#bookMarks .mk').length)
  say(hl === 0, 'пока читают выжимку справа, слева на странице ничего не подсвечено (меток ' + hl + ')')
  // 🔴 Кнопки «читать вслух» больше нет: «даже я забываю нажимать эту кнопку, никто её
  //  нажимать не будет». Микрофон открыт сразу, кнопка осталась одна — обход для тех,
  //  кто читает про себя.
  // ⚠️ ПЕРЕНАЦЕЛЕНО 13.08 (второй разбор). Обход искали на экране-подводке, где он и
  //  стоял — но нажатие там терялось: через пять секунд readAloud перерисовывает тело
  //  панели, а перерисовка чистит очередь нажатий. Теперь кнопка живёт на самом экране
  //  чтения, и проверять её надо ТАМ, иначе приёмка снова благословит мёртвую кнопку.
  say(!c1.кнопки.includes('aloud'), 'читать вслух не надо просить кнопкой: ' + c1.кнопки.join(' '))
  // Живой экран чтения в приёмке не поймать: микрофона у headless нет, и readAloud уходит
  // в запасную ветку «микрофон не поднялся». Поэтому проверяем ДВЕ вещи по отдельности.
  // Первая — в разметке самого экрана чтения обход есть (именно он пропал утренним кругом).
  const исходник = await (await fetch('http://localhost:8781/kniga')).text()
  const читалка = исходник.match(/rdbar[\s\S]{0,420}?crow[\s\S]{0,300}?<\/div>'\)/)
  say(!!читалка && /data-act=read/.test(читалка[0]),
    'в разметке экрана чтения есть обход «прочитал про себя»')
  // Вторая — какой бы экран ни выпал ребёнку, выход с него есть всегда.
  const выход = await (async () => {
    for (let i = 0; i < 40; i++) {
      const c = await ctl()
      if (c.идёт && c.этап === 'read' && c.кнопки.length) return c.кнопки
      await page.waitForTimeout(150)
    }
    return null
  })()
  say(!!выход, 'с экрана чтения есть выход: ' + (выход ? выход.join(' ') : 'кнопок не появилось'))
  ...
  // ⚠️ 13.08: кнопки «читать вслух» больше нет — readAloud стартовал сам, когда Аня
  // сказала «читай». Расшифровку отдаём сейчас, после замеров начальной мозаики: шов
  // внутри readAloud её дожидается (в боевом уроке эта ветка не работает).
  ...
  // 🔴 ПЕРЕВЁРНУТО 13.08: след голоса ребёнка живёт СПРАВА, в коробке выжимки, а не на
  //  странице. Владимир: «ты должен одновременно и налево и направо смотреть — это
  //  неправильно». Значит меряем: слева линии НЕТ, справа предложения отмечены как
  //  прочитанные.
  const слева = await page.evaluate(() => document.querySelectorAll('#bookMarks .mk.line').length)
  say(слева === 0, 'пока читает ребёнок, страница слева не подчёркивается (линий ' + слева + ')')
  // Правый след голоса — это мозаика: она открывается по ходу чтения и проверена выше
  // («на половине прочитанного открыта половина мозаики»).
...
if (ct) {
  ...
  // 🔴 «Пусть сама переключает: даёт время посмотреть, потом включает следующий».
  //  Значит экран обязан пройти ВСЕ топики БЕЗ единого нажатия. Кнопка при этом
  //  остаётся — она просто перескакивает ожидание, и это проверяется ниже.
  // начинаем считать с первого топика, иначе первый успевает пройти до старта опроса
  ...
  for (let i = 0; i < 600; i++) {
    ...
    const n = await page.evaluate(() => (window.__kniga.topics() || {}).текущий)
    if (n != null) виденные.add(n)
    await page.waitForTimeout(120)
  }
  say(виденные.size >= t0.всего,
    'экран сам прошёл по каждому топику, без нажатий (' + виденные.size + ' из ' + t0.всего + ')')
  say(нажатий === 0, 'и кнопку для этого жать не пришлось')
...
if (DRILLED) {
  ...
  if (cd) {
    ...
    if (ca) {
      ...
      // ⚠️ 13.08: опрашивать раз в 150 мс стало нельзя. Паузы урока сократились (правка
      // «она замолчала»), а синтез в приёмке — тишина, поэтому шаг пролетает между
      // замерами. Считаем ПИК числа меток наблюдателем, он не пропустит вспышку.
      await page.evaluate(() => {
        window.__пикМеток = document.querySelectorAll('#bookMarks .mk').length
        const t = document.querySelector('#bookMarks')
        window.__набл = new MutationObserver(() => {
          window.__пикМеток = Math.max(window.__пикМеток, t.querySelectorAll('.mk').length)
        })
        window.__набл.observe(t, { childList: true, subtree: true })
      })
      let hl = 0
      for (let i = 0; i < 60; i++) {
        const c = await ctl()
      ...
      hl = await page.evaluate(() => { window.__набл.disconnect(); return window.__пикМеток })
