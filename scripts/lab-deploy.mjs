...
// ⚠️ Общие данные читают ВСЕ сборки: правка в ЧАРЛИ через них меняет и то, что
...
if (общие.length) {
  console.log('\n  ⚠️ ТРОГАЕМ ОБЩИЕ ДАННЫЕ — изменится и в АЛЬФЕ, и в БРАВО:')
...
// ── 5. коммит-снимок ───────────────────────────────────────────────────────
const версии = { 'ЧАРЛИ': buildVersion('kniga.html'), 'БРАВО': buildVersion('kniga-v23.html'), 'АЛЬФА': buildVersion('kniga-v1.html') }
let sha = 'без коммита'
if (!NOCOMMIT) {
  console.log('\n── снимок в git ──')
  const шапка = `## ${ID}\n\n`
    ...
    + `- **зачем:** ${NOTE}\n`
    + `- **сборки:** ЧАРЛИ ${версии['ЧАРЛИ']} · БРАВО ${версии['БРАВО']} · АЛЬФА ${версии['АЛЬФА']}\n`
    ...
    + (общие.length ? `- ⚠️ затронуты ОБЩИЕ данные — меняется и в АЛЬФЕ, и в БРАВО\n` : '')
