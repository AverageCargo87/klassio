...
const служебная = (t) => АНОНС.test(norm(t.txt)) || парная(t)
// 🔴 Карточка вида «Мегарон и фреска» объясняет СРАЗУ ДВА слова, и её объяснение
// неизбежно повторяет оба — задание «что это значит?» решалось бы угадыванием.
// На экране топиков она нужна (там учитель разбирает оба слова по учебнику),
// а в тренажёре у каждого слова есть своё задание по отдельности.
function парная (t) {
  const m = String(t.term || '').match(/^(.+?)\s+и\s+(.+)$/i)
  if (!m) return false
  const низ = String(t.txt || '').toLowerCase()
  const есть = (w) => низ.includes(w.toLowerCase().slice(0, Math.max(4, w.length - 2)))
  return есть(m[1]) && есть(m[2])
}
